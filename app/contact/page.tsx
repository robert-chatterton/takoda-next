    ...
    <div className='max-w-2xl mx-auto px-6 py-24'>
      ...
      <section className='mb-16'>
        ...
        <div className='space-y-4'>
          ...
          <ContactInfoLink label='Instagram' value='@takodadionne' href={INSTAGRAM} />
          <ContactInfoLink label='Facebook' value='Takoda Dionne' href={FACEBOOK} />
          <ContactInfoLink label='Youtube' value='@TakodaDionneMusic' href={YOUTUBE_CHANNEL} />
        </div>
      </section>
...
function ContactInfoLink({ label, value, href }: { label: string; value: string; href: string; }) {
  return (
    <p className='flex items-center justify-between text-neutral-300'>
      <span className='text-neutral-500'>{label}</span>
      <Link target={'_blank'} rel={'noreferrer'} href={href} className='underline-offset-4 hover:underline'>
        {value}
      </Link>
    </p>
  );
